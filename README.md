...
| **setting** the modification time of a directory | `utimensat` on a directory reports `EISDIR`, so `std::filesystem::last_write_time(dir, t)` throws `filesystem error: in last_write_time: Is a directory`. **Reading** it is unaffected and correct | `kal_fs_set_modified` takes a `kal_file` and openkal has neither a `kal_dir` form of it nor a form that takes a name, so this port opens the name as a file — which a directory refuses. Note that libc++ gives both overloads of `last_write_time` the same name in the message it throws, so the text does not say which of the two failed: the reading overload is `stat` and works on a directory. A caller using a lock directory's timestamp reads it to decide staleness and writes it to refresh the lock, and only the second fails. |
